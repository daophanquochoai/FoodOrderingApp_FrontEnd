...
const ClientBreadcrumb: React.FC<ClientBreadItemProps> = ({ title, items }) => {
    const breadcrumbItems = items?.map((item) => {
        if (typeof item == 'string') {
            return {
                title: <span className="font-mono text-base tracking-wider">{item}</span>,
            };
        }
        ...
        return {
            title: (
                <Link to={item.to}>
                    <span className="font-mono text-base tracking-wider">{item.label}</span>
                </Link>
            ),
        };
    });
    ...
    return (
        <div className="bg-[#ffffff] py-5 md:py-7 lg:py-9 flex flex-col items-center justify-center gap-1 ">
            <h2 className="text-xl md:text-2xl font-kanit font-medium tracking-wide">{title}</h2>
            <div>
