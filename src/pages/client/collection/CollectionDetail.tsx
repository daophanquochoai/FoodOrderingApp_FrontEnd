...
const foods: Food[] = [
  ...
  },
  {
    ...
    image:
      "https://thumbs.dreamstime.com/b/vietnamese-iced-coffee-refreshing-authentic-coffee-delight-vietnamese-iced-coffee-popular-beverage-captured-photo-309197334.jpg",
    status: true,
...
const CollectionDetail = () => {
  ...
  // const categoryTree = buildCategoryTree(categories, Number(categoryId));
  const categoryDetail = categories.find(category => category.id == Number(categoryId));

  const categorySubIds =  getAllSubCategoryIds(categories, Number(categoryId));

  const categorySubs = categorySubIds.length > 0 ? categories.filter(category => categorySubIds.includes(category.id)) : [];
  // console.log(categorySubs);

  return (
    <div>
      <ClientBreadcrumb title={String(categoryDetail?.name)} items={[{ label: "Home", to: "/" }]} />
      <div className='py-4'>
