...
function* handleFetchCategories() {
    try {
        ...
        const homepageCategoriesResponse = yield call(
            [categoryHomepageApi, categoryHomepageApi.getCategoryHomepage],
            token
        );
        ...
        const availableCategories = allCategories.filter(
            (category) => !dealOfWeekCategoryIds.includes(category.id)
        );
...
function* handleAddHomepageCategories(action: ReturnType<typeof actions.addCategoryToHomepage>) {
    try {
        ...
        yield put(
            common.actions.setSuccessMessage(`Added ${record.name || 'category'} to homepage`)
        );
    } catch (error) {
...
function* handleFetchLatestProducts() {
    try {
        ...
        const token = getCookies('access_token');
        const latestProductsResponse = yield call(
            [foodHomepageApi, foodHomepageApi.getLatestFoodHomepage],
            token
        );
        ...
        const availableProducts = allProducts.filter(
            (product) => !dealOfWeekProductIds.includes(product.id)
        );
...
function* handleFetchDealOfWeek() {
    try {
        ...
        const token = getCookies('access_token');
        const dealOfWeekProductsResponse = yield call(
            [foodHomepageApi, foodHomepageApi.getDealOfTheWeekHomepage],
            token
        );
...
function* handleDeleteCategoryHomepage(
    action: ReturnType<typeof actions.removeCategoryFromHomepage>
) {
    try {
        const record = action.payload;
        const token = getCookies('access_token');
